...
namespace DupesCuisine
{
    class RegisterStrings
    {
        ...
        public static void MakeFoodStrings(string id, string name, string desc, string RECIPEDESC = "")
        {
            Strings.Add($"STRINGS.ITEMS.FOOD.{id.ToUpperInvariant()}.NAME", name);
            Strings.Add($"STRINGS.ITEMS.FOOD.{id.ToUpperInvariant()}.DESC", desc);
            if (RECIPEDESC != "")
                Strings.Add($"STRINGS.ITEMS.FOOD.{id.ToUpperInvariant()}.RECIPEDESC", RECIPEDESC);
        }
